...
def predict():
    try:
        ...
        moving_avg = original_data['close'].rolling(window=20).mean()
        
        # Create results DataFrame with all necessary data
        results_df = pd.DataFrame({
            'time': original_data['time'].tail(30),
            'actual_close': original_data['close'].tail(30),
            'predicted_close': predictions[-30:],
            'volatility': volatility.tail(30),
            'moving_avg': moving_avg.tail(30)
        })
        ...
        return jsonify({
            ...
            'predictions': results,
            'summary': {
                'mean_price': float(original_data['close'].mean()),
                'max_price': float(original_data['close'].max()),
                'min_price': float(original_data['close'].min()),
