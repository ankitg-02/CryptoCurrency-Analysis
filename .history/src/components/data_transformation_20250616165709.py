...
class CryptoDataTransformer:
    ...
    def transform_data(self):
        try:
            ...
            # Feature engineering with smaller windows
            df['daily_return'] = df['close'].pct_change()
            df['volatility'] = df['daily_return'].rolling(window=7).std()  # Reduced from 30
            df['MA_20'] = df['close'].rolling(window=20).mean()  # Changed from 50
            df['MA_50'] = df['close'].rolling(window=50).mean()  # Changed from 200
            
            logger.info(f"Shape after feature engineering: {df.shape}")
            
            # Forward fill NaN values instead of dropping
